...
game:GetService('RunService').RenderStepped:Connect(function()
    if _G.Disabled then
        ...
        if charactersFolder then
            local zombiesFolder = charactersFolder:FindFirstChild("Zombies")
            if zombiesFolder then
                modifyCharacters(zombiesFolder)
            end
        end
    end
end)
...
local options = {
    ...
    camera_cframe = true, -- aimlock, easier to detect, does not obey the smoothness setting but locks instantly
